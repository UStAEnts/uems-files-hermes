...
import {
    FileMessage as EM,
    FileResponse as ER,
    FileMessageValidator,
    FileResponseValidator,
    FileBindingMessageValidator,
    FileBindingResponseValidator,
    has,
...
fs.readFile(process.env.UEMS_HERMES_CONFIG_LOCATION ?? path.join(__dirname, '..', '..', 'config', 'configuration.json'), {encoding: 'utf8'})
    ...
    .then(() => (new Promise<void>((resolve, reject) => {
        ...
        const discoverMessageValidator = new DiscoveryMessageValidator();
        const discoverResponseValidator = new DiscoveryResponseValidator();
        ...
        const validateIncoming = async (data: any) => {
            ...
            try {
                const discover = await discoverMessageValidator.validate(data);
        ...
        const validateOutgoing = async (data: any) => {
            ...
            try {
                const discover = await discoverResponseValidator.validate(data);
                if (discover) return true;
            } catch (e) {
            }
        ...
        messager = new RabbitNetworkHandler<EM.FileMessage | DiscoveryMessage.DiscoveryDeleteMessage,
            EM.CreateFileMessage,
            EM.DeleteFileMessage,
            EM.ReadFileMessage | DiscoveryMessage.DiscoveryDeleteMessage,
            EM.UpdateFileMessage,
            // @ts-ignore
            ER.FileServiceReadResponseMessage | ER.FileResponseMessage | DiscoveryResponse.DiscoveryDeleteResponse>
        (
